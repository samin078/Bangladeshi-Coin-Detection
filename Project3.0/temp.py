...
# Constants
TEMPLATE_IMG = "temp/one-o-1-f.jpg"
ANGLE_STEP = 30
THRESHOLD = 130
THRESHOLD_TYPE = cv2.THRESH_BINARY_INV
CLOSE_ITERATIONS = 5
TEMPLATE_SIZE = 100
INPUT_IMAGE = "one.jpeg"
LABELED_IMAGE = "coins_withtwoLabeled.bmp"
LABEL = "2tk"
MATCH_THRESHOLD = 0.3
CANDIDATES_MIN_AREA = 1600  # Adjust as needed
...
# Constants (unchanged from your original script)

# Function to create a file dialog for selecting the input image
def select_input_image():
    root = tk.Tk()
    root.withdraw()  # Hide the main window
    file_path = filedialog.askopenfilename(title="Select Input Image", filetypes=[("Image files", "*.jpg;*.jpeg;*.png")])
    return file_path
...
def main():
    ...
    # Rest of your existing main function goes here
    # Replace INPUT_IMAGE with input_image_path
    ...
    candidates = get_candidates(image, mask)
    save_candidates(candidates)
    match_candidates(templates, candidates)
    for candidate in candidates:
        print(candidate.score)
    labeled_img = label_coins(image, candidates, MATCH_THRESHOLD, LABEL)
